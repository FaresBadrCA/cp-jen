...
Jen Configuration file specification:
...
class InputVariable():
    ...
    def set_value(self):
        ...
        if 'type' not in self.props: raise Exception("Variable type not specified.")
        if self.props['type'] == 'int':
            self.value = random.randint(self.get_property('min'), self.get_property('max'))
        elif self.props['type'] == 'float':
            self.value = random.uniform(self.get_property('min'), self.get_property('max'))
        elif self.props['type'] == 'str':
            k = random.randint(self.get_property('min'), self.get_property('max'))
    ...
    def get_property(self, prop):
        ...
        if (type(prop_val) == str): return StressTester.var_dict[prop_val].value
        else: return prop_val
...
class InputGroup():
    ...
    def add_variable(self, var):
        ...
        """
        if (var not in StressTester.var_dict): StressTester.var_dict[var] = InputVariable(var)
        input_variable = StressTester.var_dict[var]
        if (var in StressTester.config): input_variable.props.update(StressTester.config[var])
    ...
    def render(self):
        ...
        for _ in range(count):
            self.reset_value()
            for out_item in self.out_list:
                if type(out_item) == str: render_list.append(out_item)
